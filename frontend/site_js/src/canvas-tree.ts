...
export function newTree(canvas: fabric.Canvas, treeRows: TreeRow[]): TreeNode {
  ...
  const rootNode = new TreeNode(rootCanvasObject);
  ...
  function addChildren(rootItem: TreeNode, rootItemId: number) {
    for (let i = 0; i < treeRows.length;) {
      ...
      if (row.Parent === rootItemId) {
        ...
        let childNode = rootItem.addChild(newObject);
...
export class TreeNode {
  ...
  constructor(canvasObject: fabric.Object) {
  ...
  addChild(canvasObject: fabric.Object): TreeNode {
    let itemNode = new TreeNode(canvasObject);
